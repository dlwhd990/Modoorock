...
import styles from "./adminProgramMissionUploadTopper.module.css";
import axios from "axios";

const AdminProgramMissionUploadTopper = ({
  loadMissionList,
  ...
  const onTopperInputHandler = (e) => {
    ...
    const newFile = new File([blob], `${fileName[0]}_filter.${fileName[1]}`, {
      type: "image",
    });
    reader.onloadend = () => {
      setTopperImage({
  ...
  return (
    <form className={styles.form}>
      ...
      <div className={`${styles.photo_container} ${styles.container}`}>
        <p className={styles.text}>토퍼 이미지</p>
        <input
          onChange={onTopperInputHandler}
          type="file"
          accept="image/jpg,image/png,image/jpeg,image/webp"
          className={styles.file_input}
        />
      ...
      <div className={`${styles.button_container} ${styles.container}`}>
        <button className={styles.button} onClick={insertTopperMission}>
          등록
        </button>
...
export default AdminProgramMissionUploadTopper;
